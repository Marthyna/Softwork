...
public class AlunoMensagens extends javax.swing.JFrame {
    ...
    LinkedList<Mensagem> listaMensagens;

    public AlunoMensagens(Aluno meuAluno, Socket cliente, ObjectOutputStream out, ObjectInputStream in, String[] alfabetosCriptografia) {
        initComponents();
    ...
    private void initComponents() {
        ...
        jLMensagens.setFont(new java.awt.Font("Calibri Light", 1, 18)); // NOI18N
        ...
        jTMensagens.setFont(new java.awt.Font("Calibri Light", 0, 14)); // NOI18N
        ...
        jTMensagens.setToolTipText("");
        jTMensagens.setRowHeight(35);
        jTMensagens.setRowMargin(10);
    ...
    private void jBDetalharActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDetalharActionPerformed
        if (jTMensagens.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(rootPane, "Selecione uma mensagem para detalhar.");
        } else {
            int x = jTMensagens.getSelectedRow();
            String selecionadaAssunto = (String) jTMensagens.getValueAt(x, 0);
            
            try {
                out.writeObject("detalhaMensagemAluno");

                mensagemRecebida = (String) in.readObject();
                if (mensagemRecebida.equals("Ok")) {
                    out.writeObject(selecionadaAssunto);
                    out.writeObject(meuAluno);

                    Mensagem mensagem = (Mensagem) in.readObject();

                    AlunoMensagemDetalhar telaDetalhar = new AlunoMensagemDetalhar(mensagem, cliente, out, in, meuAluno);
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        model = (DefaultTableModel) jTMensagens.getModel();
        model.setNumRows(0);
        ...
        try {
            ...
            if (mensagemRecebida.equals("Ok")) {
                ...
                if (mensagemRecebida.equals("temMensagens")) {
                    listaMensagens = (LinkedList<Mensagem>) in.readObject();

                    for (int x = 0; x < listaMensagens.size(); x++) {
                        model.addRow(new Object[]{listaMensagens.get(x).getAssunto(),listaMensagens.get(x).getEmpresa().getNomeFantasia()});
                    }
                } else if (mensagemRecebida.equals("naoTemMensagens")) {
                    JOptionPane.showMessageDialog(rootPane, "Não há mensagens na caixa de entrada.");
                }
