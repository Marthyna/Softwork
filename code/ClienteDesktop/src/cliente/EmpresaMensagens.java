...
public class EmpresaMensagens extends javax.swing.JFrame {
    ...
    public EmpresaMensagens(Empresa minhaEmpresa, Socket cliente, ObjectOutputStream out, ObjectInputStream in, String[] alfabetosCriptografia) {
        initComponents();
    ...
    private void jBDetalharActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDetalharActionPerformed
        if (jTMensagens.getSelectedRow() == -1) {
            ...
            String assunto = (String) jTMensagens.getValueAt(x, 1);

            try {
                out.writeObject("detalhaMensagemEmpresa");

                mensagemRecebida = (String) in.readObject();
                if (mensagemRecebida.equals("Ok")) {
                    out.writeObject(assunto);
                    out.writeObject(minhaEmpresa);

                    Mensagem minhaMsg = (Mensagem) in.readObject();

                    EmpresaMensagemDetalhar telaMensagemDetalhar = new EmpresaMensagemDetalhar(minhaMsg, cliente, out, in, minhaMsg.getAluno());
                    telaMensagemDetalhar.setVisible(true);
            ...
            if (mensagemRecebida.equals("Ok")) {
                ...
                if (mensagemRecebida.equals("temMensagens")) {
                    ...
                    for (int x = 0; x < listaMensagens.size(); x++) {
                        model.addRow(new Object[]{listaMensagens.get(x).getAluno().getNomeCompleto(), listaMensagens.get(x).getAssunto()});
                    }
                } else if (mensagemRecebida.equals("naoTemMensagens")) {
                    JOptionPane.showMessageDialog(rootPane, "Nenhuma mensagem recebida ainda.");
                }
